    ...
    <div className="max-w-7xl mx-auto py-10 px-2 md:px-8">
      <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {filteredItems.map(item => (
          <div
            key={item.item_id}
            className="relative rounded-2xl shadow-xl overflow-hidden flex flex-col justify-end min-h-[340px] bg-gray-100 group transition-all duration-300 hover:scale-105 hover:shadow-2xl"
            style={{
          ...
          >
            ...
            <div className="relative z-10 p-6 flex flex-col h-full justify-end">
              <h2 className="text-2xl font-bold text-white mb-2 drop-shadow">{item.item_name}</h2>
              <p className="text-xl text-yellow-300 font-semibold mb-4 drop-shadow">₹{item.item_price}</p>
              <div className="flex gap-3">
                <button className="bg-green-500 hover:bg-green-600 text-white px-5 py-2 rounded-lg font-semibold shadow transition">Buy</button>
                <button className="bg-blue-500 hover:bg-blue-600 text-white px-5 py-2 rounded-lg font-semibold shadow transition">Add to Cart</button>
              </div>
            </div>
          </div>
        ))}
